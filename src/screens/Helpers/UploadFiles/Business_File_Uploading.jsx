...
const Business_File_Uploading = ({imageData,IdType,CountryId,AdditionalDocument,Update_Step,UploadDocument}) => {
    ...
    const onSelectAdditionalFile = (event) => {
      if(IdDocumentId != ''){
        if(OtherIdDocumentId == "" && IdDocumentId == 7966){
          ...
          //               ImageType : event.target.files[0].name?.split('.').pop().toLowerCase()//?.toLowerCase()?.lastIndexOf(".")
          //             }
          //             console.log(imgfileObject,"imgfileObject")
          ...
          //                   const updatedImages = [...document.ImageArray, ...imgfileObject];
          //                   const updatedFile = [...document.ImageFileArray, ...imagesArray];
  ...
  useEffect(()=>{
    // setBusinessDataImageFront(selectedImagesFront)
    // setBusinessDataImageBack(selectedImagesBack)
    imageData(selectedFileImagesFront, selectedFileImagesBack)
  },[selectedImagesFront,selectedImagesBack])

  useEffect(()=>{
    if(Update_Step=='BsixthStep' && UploadDocument == true){
        AdditionalDocument([])
        setImageArray()
    }if(Update_Step=='BsixthStep'){
            ...
            selectedAdditionalImages.map((image, index) => {
              return (
                <div key={image} className="image position relative">
                  ...
                  <img src={image} height="100" width="200" className="hoverView uploadedImage roundedCorner" alt="upload" />
                  <div class="overlay simple-overlay roundedCorner">
                  <div className="mainBtnClose" onClick={() => deleteHandlerAddtional(image,index)}>
...
export default Business_File_Uploading;
